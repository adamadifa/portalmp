<x-app-layout>
    ...
    <!-- Header & Navigation -->
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Opname Gudang Logistik</h2>
        <p class="text-sm text-gray-500 mt-1">Mengelola data opname fisik barang di gudang logistik.</p>
    </div>

    <!-- Navigation Tabs -->
    @include('layouts.navigation_mutasigudanglogistik')

    <!-- Filter Card aligned in 1 single row -->
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
        <form action="{{ route('opgudanglogistik.index') }}" method="GET">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
        <div class="p-5 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-[#294C9A] to-[#1E3A70] text-white">
            <div class="flex items-center gap-2">
                ...
                <h3 class="font-bold text-base">Data Opname Gudang Logistik</h3>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-xs text-left text-gray-600">
                <thead class="text-xs uppercase bg-gray-50 text-gray-700 border-b border-gray-200">
                    <tr>
                        <th class="px-4 py-3 font-bold">KODE OPNAME</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse ($opname as $d)
                        <tr class="hover:bg-gray-50/80 transition">
                            <td class="px-4 py-3 font-bold text-[#294C9A] font-mono">{{ $d->kode_opname }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    @can('opgudanglogistik.edit')
                                        <button type="button" onclick="openModalEdit('{{ Crypt::encrypt($d->kode_opname) }}')" class="p-1 text-emerald-600 hover:text-emerald-800 transition" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </button>
                                    @endcan

                                    @can('opgudanglogistik.show')
                                        <button type="button" onclick="openModalShow('{{ Crypt::encrypt($d->kode_opname) }}')" class="p-1 text-sky-600 hover:text-sky-800 transition" title="Detail">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                        </button>
                                    @endcan

                                    @can('opgudanglogistik.delete')
                                        <form method="POST" action="{{ route('opgudanglogistik.delete', Crypt::encrypt($d->kode_opname)) }}" class="inline deleteform">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-gray-400">
                                ...
                                Data Opname Gudang Logistik tidak ditemukan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <script>
        function openModalCreate() {
            $('#modalTitle').text('Tambah Data Opname Gudang Logistik');
            ...
            $.get("{{ route('opgudanglogistik.create') }}", function(data) {
                $('#modalBody').html(data);
            });
        }

        function openModalEdit(kode_opname) {
            $('#modalTitle').text('Edit Data Opname Gudang Logistik');
            $('#modalBody').html('<div class="flex items-center justify-center py-8"><svg class="w-8 h-8 text-[#294C9A] animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></div>');
            $('#modalDialog').removeClass('hidden');

            let url = "{{ route('opgudanglogistik.edit', ':kode_opname') }}".replace(':kode_opname', kode_opname);
            $.get(url, function(data) {
                $('#modalBody').html(data);
            });
        }

        function openModalShow(kode_opname) {
            $('#modalTitle').text('Detail Opname Gudang Logistik');
            $('#modalBody').html('<div class="flex items-center justify-center py-8"><svg class="w-8 h-8 text-[#294C9A] animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></div>');
            $('#modalDialog').removeClass('hidden');

            let url = "{{ route('opgudanglogistik.show', ':kode_opname') }}".replace(':kode_opname', kode_opname);
            $.get(url, function(data) {
                $('#modalBody').html(data);
            });
        }
