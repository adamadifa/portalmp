<form action="{{ route('laporangudanglogistik.cetakkartugudang') }}" method="POST" id="frmKartugudang" target="_blank" class="space-y-6 pt-1">
    @csrf
    ...
    <div class="c-fl-group">
        ...
        <select name="kode_barang_kartugudang" id="kode_barang_kartugudang" class="select2Kodebarangkartugudang">
            <option value=""></option>
            @foreach ($barang as $d)
                <option value="{{ $d->kode_barang }}">{{ $d->kode_barang }} | {{ strtoupper($d->nama_barang) }}</option>
            @endforeach
        </select>
        <label for="kode_barang_kartugudang" class="c-fl-label">Pilih Barang *</label>
    </div>
