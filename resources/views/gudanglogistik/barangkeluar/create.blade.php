<form action="{{ route('barangkeluargudanglogistik.store') }}" method="post" id="formcreatebarangkeluargudanglogistik" class="space-y-4">
    @csrf

    <div id="top_fields_grid" class="grid grid-cols-1 md:grid-cols-12 gap-3">
        <div id="col_no_bukti" class="md:col-span-4">
            <div class="c-fl-group">
                ...
                <input type="text" name="no_bukti" id="no_bukti" class="fi" placeholder="No. Bukti Pengeluaran" autocomplete="off" />
                <label for="no_bukti" class="c-fl-label">No. Bukti Pengeluaran *</label>
            </div>
        </div>

        <div id="col_tanggal" class="md:col-span-4">
            <div class="c-fl-group">
                <span class="c-fl-icon">
        ...
        <div id="col_jenis" class="md:col-span-4">
            <div class="c-fl-group">
                <span class="c-fl-icon">
        ...
        <div id="cabang_select" class="md:col-span-3 hidden">
            <div class="c-fl-group">
                <span class="c-fl-icon">
...
<script>
$(function() {
    const formCreate = $("#formcreatebarangkeluargudanglogistik");
    ...
    formCreate.find("#kode_jenis_pengeluaran").change(function() {
        let val = $(this).val();
        if (val === 'CBG') {
            ...
            formCreate.find("#kode_cabang").val('');
    ...
    $("#tambahproduk").click(function(e) {
        ...
        const nama_barang = formCreate.find("#kode_barang option:selected").text();
        const jml = formCreate.find("#jumlah").val();
        const kode_cbg = formCreate.find("#kode_cbg").val();
        const nama_cbg = formCreate.find("#kode_cbg option:selected").text();
        const ket = formCreate.find("#keterangan").val();
        ...
        if (kode_barang == "") {
            ...
            let row = `<tr id="index_${kode_barang}">
                <td class="px-3 py-2">
                    ...
                    <span class="font-medium text-gray-900">${kode_barang}</span>
                </td>
            ...
            formCreate.find("#kode_barang").val('').trigger('change');
            formCreate.find("#jumlah").val('');
            formCreate.find("#kode_cbg").val('');
            formCreate.find("#keterangan").val('');
        }
    });
    ...
    formCreate.submit(function() {
        ...
        if (no_bukti == "") {
            ...
            return false;
        } else if (!formCreate.find("#aggrement").is(":checked")) {
