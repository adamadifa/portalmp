<x-app-layout>
    ...
    <!-- Header & Navigation -->
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Barang Masuk Gudang Logistik</h2>
        <p class="text-sm text-gray-500 mt-1">Mengelola data penerimaan barang masuk ke gudang logistik.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
        <form action="{{ route('barangmasukgudanglogistik.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                ...
                <div class="md:col-span-4">
                    <div class="c-fl-group">
                        ...
                        <input type="text" name="no_bukti_search" id="no_bukti_search" value="{{ Request('no_bukti_search') }}" class="fi" placeholder="No. Bukti Pemasukan" autocomplete="off" />
                        <label for="no_bukti_search" class="c-fl-label">No. Bukti Pemasukan</label>
                    </div>
                </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
        <div class="p-5 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-[#294C9A] to-[#1E3A70] text-white">
            <div class="flex items-center gap-2">
                ...
                <h3 class="font-bold text-base">Data Barang Masuk</h3>
            </div>
            ...
            <table class="w-full text-xs text-left text-gray-600">
                <thead class="text-xs uppercase bg-gray-50 text-gray-700 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 font-bold text-center">TGL PEMBELIAN</th>
                        <th class="px-4 py-3 font-bold text-center">TGL DITERIMA</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse ($barangmasuk as $d)
                        <tr class="hover:bg-gray-50/80 transition">
                            ...
                            <td class="px-4 py-3 font-medium text-gray-900 max-w-[200px] truncate">{{ $d->nama_supplier ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    @can('barangmasukgl.delete')
                                        <form method="POST" action="{{ route('barangmasukgudanglogistik.delete', Crypt::encrypt($d->no_bukti)) }}" class="inline deleteform">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-8 text-center text-gray-400">
                                ...
                                Data Barang Masuk Gudang Logistik tidak ditemukan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <script>
        ...
        function openModalCreate() {
            $('#modalTitle').text('Tambah Data Barang Masuk Gudang Logistik');
            ...
            $.get("{{ route('barangmasukgudanglogistik.create') }}", function(data) {
                $('#modalBody').html(data);
            });
        }

        function openModalEdit(no_bukti) {
            $('#modalTitle').text('Edit Data Barang Masuk Gudang Logistik');
            $('#modalBody').html('<div class="flex items-center justify-center py-8"><svg class="w-8 h-8 text-[#294C9A] animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></div>');
            $('#modalDialog').removeClass('hidden');

            let url = "{{ route('barangmasukgudanglogistik.edit', ':no_bukti') }}".replace(':no_bukti', no_bukti);
        ...
        function openModalShow(no_bukti) {
            $('#modalTitle').text('Detail Barang Masuk Gudang Logistik');
            $('#modalBody').html('<div class="flex items-center justify-center py-8"><svg class="w-8 h-8 text-[#294C9A] animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></div>');
            $('#modalDialog').removeClass('hidden');

            let url = "{{ route('barangmasukgudanglogistik.show', ':no_bukti') }}".replace(':no_bukti', no_bukti);
            $.get(url, function(data) {
                $('#modalBody').html(data);
            });
        }
