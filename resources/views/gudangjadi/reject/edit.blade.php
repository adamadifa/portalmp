...
<form action="{{ route('rejectgudangjadi.update', Crypt::encrypt($reject->no_mutasi)) }}" method="POST" id="formeditReject" class="space-y-4" novalidate>
    @csrf

    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div class="fl-group">
            <input type="text" name="no_mutasi" id="no_mutasi" readonly value="{{ $reject->no_mutasi }}" placeholder=" " class="fl-input" />
            <label class="fl-label" for="no_mutasi">No. Reject (Otomatis)</label>
        </div>
        <div class="fl-group">
            <input type="text" name="tanggal" id="tanggal" value="{{ $reject->tanggal }}" required placeholder=" " class="fl-input flatpickr-date" autocomplete="off" />
