...
<form action="{{ route('lainnyagudangjadi.store') }}" method="POST" id="formcreateLainnya" class="space-y-2.5" novalidate>
    @csrf

    <!-- Row 1: No Dokumen & Tanggal -->
    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
        <div class="fl-group">
            <input type="text" name="no_mutasi" id="no_mutasi" readonly placeholder=" " class="fl-input" />
            <label class="fl-label" for="no_mutasi">No. Dokumen (Otomatis)</label>
        </div>
        <div class="fl-group">
            <input type="text" name="tanggal" id="tanggal" required placeholder=" " class="fl-input flatpickr-date" autocomplete="off" />
    ...
    <!-- Row 2: Keterangan -->
    <div class="fl-group">
        <input type="text" name="keterangan" id="keterangan" required placeholder=" " class="fl-input" />
    ...
    <div class="flex flex-col gap-1.5">
        ...
        <div class="grid grid-cols-2 gap-2">
            <label class="relative flex items-center justify-center p-2.5 rounded-xl border border-gray-200 bg-gray-50 cursor-pointer transition select-none hover:bg-gray-100/50 has-[:checked]:border-[#294C9A] has-[:checked]:bg-[#EEF2FF] has-[:checked]:text-[#294C9A]">
                <input type="radio" name="in_out" value="I" required class="sr-only" />
            ...
            <label class="relative flex items-center justify-center p-2.5 rounded-xl border border-gray-200 bg-gray-50 cursor-pointer transition select-none hover:bg-gray-100/50 has-[:checked]:border-[#294C9A] has-[:checked]:bg-[#EEF2FF] has-[:checked]:text-[#294C9A]">
                <input type="radio" name="in_out" value="O" required class="sr-only" />
                <span class="text-xs font-bold flex items-center gap-1.5">
...
<script>
    $(function() {
        ...
        function addProduk() {
            ...
            const nama_produk = formCreate.find("#kode_produk option:selected").text().split(' - ')[1] || '';
        ...
        formCreate.find("#tambahproduk").click(function(e) {
            ...
            if (kode_produk == "") {
                ...
                if (formCreate.find('#index_' + kode_produk).length > 0) {
                    Swal.fire({
                        title: "Oops!",
                        text: "Data Sudah Ada!",
                        icon: "warning",
        ...
        formCreate.on('click', '.btn-delete', function(e) {
            e.preventDefault();
        ...
        formCreate.find('.agreement').change(function() {
            if (this.checked) {
                formCreate.find("#saveButton").removeClass('hidden');
            } else {
                formCreate.find("#saveButton").addClass('hidden');
            }
        });
    });
</script>
