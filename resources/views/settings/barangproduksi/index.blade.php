<x-app-layout>
    <x-slot name="header">
        Barang Produksi Management
    </x-slot>

    <!-- Header Actions -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Master Data Barang Produksi</h2>
            <p class="text-sm text-gray-500 mt-1">Manage production items, seasoning elements, packaging, and source departments.</p>
        </div>
        <!-- Breadcrumbs -->
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 text-xs font-semibold text-gray-500">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="text-[#294C9A] font-semibold">Barang Produksi</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10">
                <div class="flex items-center gap-2 text-white">
                    ...
                    <h3 class="text-sm font-semibold">Data Master Barang Produksi</h3>
                </div>
                @can('barangproduksi.create')
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-xs font-semibold uppercase tracking-wider bg-[#294C9A] text-white">
                            ...
                            <th class="py-3 px-4" style="width: 15%;">Kode Barang</th>
                            <th class="py-3 px-4" style="width: 35%;">Nama Barang</th>
                            <th class="py-3 px-4" style="width: 10%;">Satuan</th>
                            <th class="py-3 px-4" style="width: 15%;">Asal Barang</th>
                            <th class="py-3 px-4" style="width: 15%;">Kategori</th>
                            <th class="py-3 px-4 text-center" style="width: 5%;">Status</th>
                            <th class="py-3 px-4 text-center" style="width: 5%;">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="text-sm divide-y divide-gray-100">
                        @forelse ($barangproduksi as $index => $d)
                            <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100/50 transition-colors">
                                <td class="py-1 px-4 text-gray-500 font-medium">
                                    {{ $barangproduksi->firstItem() + $index }}
                                </td>
                                ...
                                <td class="py-1 px-4 text-gray-600">
                                    {{ textUpperCase($d->satuan) }}
                                </td>
                                <td class="py-1 px-4 text-xs text-gray-700">
                                    {{ $asal_barang_produksi[$d->kode_asal_barang] ?? '-' }}
                                </td>
                                <td class="py-1 px-4 text-xs font-semibold text-[#294C9A]">
                                    {{ $kategori_barang_produksi[$d->kode_kategori] ?? '-' }}
                                </td>
                                ...
                                <td class="py-1 px-4">
                                    <div class="flex items-center justify-center gap-3">
                                        ...
                                        <button onclick="openModalEdit('{{ Crypt::encrypt($d->kode_barang_produksi) }}')" class="text-emerald-600 hover:text-emerald-900 p-1.5 hover:bg-emerald-50 rounded-lg transition" title="Edit Barang Produksi">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </button>
                                        @endcan
                                        <!-- Delete -->
                                        @can('barangproduksi.delete')
                                        <button type="button" class="btn-delete text-red-600 hover:text-red-950 p-1.5 hover:bg-red-50 rounded-lg transition" data-id="{{ Crypt::encrypt($d->kode_barang_produksi) }}" data-name="{{ $d->nama_barang }}" title="Delete Barang Produksi">
                            ...
                            <tr>
                                <td colspan="8" class="py-12 px-6 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span>No production items registered in the database.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <div id="modalCreate" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-xl w-full max-w-lg overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalCreateContentWrapper">
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-white">Tambah Barang Produksi</h3>
    ...
    <div id="modalEdit" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-xl w-full max-w-lg overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalEditContentWrapper">
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-white">Edit Barang Produksi</h3>
    ...
    @push('myscript')
        <script>
            ...
            function openModalCreate() {
                ...
                $('#loadCreateContent').load('/barangproduksi/create', function() {
                    initFormValidation('#formCreateBarangProduksi');
                });
            }

            function openModalEdit(id) {
                openModal('modalEdit');
                $('#loadEditContent').load('/barangproduksi/' + id + '/edit', function() {
                    initFormValidation('#formEditBarangProduksi');
                });
            }

            // Real-time validation for Barang Produksi forms
            ...
            $(document).on('click', '.btn-delete', function(e) {
                ...
                Swal.fire({
                    title: 'Hapus Barang Produksi?',
                    text: "Apakah Anda yakin ingin menghapus barang produksi '" + name + "' secara permanen?",
                    icon: 'warning',
