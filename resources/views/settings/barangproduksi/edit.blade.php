...
<form action="{{ route('barangproduksi.update', Crypt::encrypt($barangproduksi->kode_barang_produksi)) }}" method="POST" id="formEditBarangProduksi" novalidate>
    @csrf
    @method('PUT')

    {{-- Row 1: Kode Barang (Readonly) & Nama Barang --}}
    <div class="grid grid-cols-2 gap-3 mb-3">
        <div class="fl-group">
            <input type="text" id="kode_barang_produksi" name="kode_barang_produksi" disabled value="{{ $barangproduksi->kode_barang_produksi }}" placeholder=" " class="fl-input">
            <label class="fl-label" for="kode_barang_produksi">Kode Barang</label>
        </div>
        <div class="fl-group">
            <input type="text" id="nama_barang" name="nama_barang" required maxlength="255" value="{{ $barangproduksi->nama_barang }}" placeholder=" " class="fl-input uppercase">
    ...
    <div class="grid grid-cols-2 gap-3 mb-3">
        <div class="fl-group">
            <input type="text" id="satuan" name="satuan" required maxlength="10" value="{{ $barangproduksi->satuan }}" placeholder=" " class="fl-input uppercase">
        ...
        <div class="fl-group">
            <select id="kode_asal_barang" name="kode_asal_barang" required class="fl-select">
                ...
                @foreach ($asal_barang_produksi as $key => $val)
                    <option value="{{ $key }}" {{ $barangproduksi->kode_asal_barang == $key ? 'selected' : '' }}>
                        {{ $val }}
                    </option>
                @endforeach
            </select>
    ...
    <div class="grid grid-cols-2 gap-3 mb-4">
        <div class="fl-group">
            <select id="kode_kategori" name="kode_kategori" required class="fl-select">
                ...
                @foreach ($list_kategori_barang_produksi as $d)
                    <option value="{{ $d['kode_kategori'] }}" {{ $barangproduksi->kode_kategori == $d['kode_kategori'] ? 'selected' : '' }}>
                        {{ $d['nama_kategori'] }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="fl-group">
            <select id="status_aktif_barang" name="status_aktif_barang" required class="fl-select">
                ...
                <option value="0" {{ $barangproduksi->status_aktif_barang === '0' ? 'selected' : '' }}>Non-Aktif</option>
            </select>
