<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">System Users</h2>
            <p class="text-sm text-gray-500 mt-1">Manage user accounts, credentials, and assign authorization roles.</p>
        </div>
        <!-- Breadcrumbs -->
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 text-xs font-semibold text-gray-500">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="text-[#294C9A] font-semibold">Users</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <!-- Filter -->
    <div class="mb-4">
        <form action="{{ route('users.index') }}" method="GET" class="flex flex-col sm:flex-row gap-2 w-full">
            <div class="relative flex-1">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                </div>
                <input type="text" name="name" value="{{ request('name') }}" class="block w-full py-3 pl-9 pr-4 text-xs text-gray-900 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#294C9A] focus:border-[#294C9A] focus:outline-none transition placeholder-gray-400 shadow-sm" placeholder="Cari Nama / Email User...">
            </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10">
                <div class="flex items-center gap-2 text-white">
                    ...
                    <h3 class="text-sm font-semibold">Data Users</h3>
                </div>
                    ...
                    <tbody class="text-sm divide-y divide-gray-100">
                        @forelse ($users as $d)
                            <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100/50 transition-colors">
                                ...
                                <td class="py-1.5 px-4 font-semibold text-gray-900">
                                    {{ $d->name }}
                                </td>
                                <td class="py-1.5 px-4 text-gray-600">
                                    {{ $d->email }}
                                </td>
                                <td class="py-1.5 px-4">
                                    @foreach($d->roles as $role)
                                        <span class="px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-indigo-700 bg-indigo-50 border border-indigo-100 rounded">
                                            {{ $role->name }}
                                        </span>
                                    @endforeach
                                </td>
                                <td class="py-1.5 px-4">
                                    <div class="flex items-center justify-center gap-3.5">
                                        <!-- Edit -->
                                        <button onclick="openModalEdit('{{ Crypt::encrypt($d->id) }}')" class="text-emerald-600 hover:text-emerald-900 p-1.5 hover:bg-emerald-50 rounded-lg transition" title="Edit User">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </button>
                                        <!-- Delete -->
                                        <button type="button" class="btn-delete text-red-600 hover:text-red-950 p-1.5 hover:bg-red-50 rounded-lg transition" data-id="{{ Crypt::encrypt($d->id) }}" data-name="{{ $d->name }}" title="Delete User">
                            ...
                            <tr>
                                <td colspan="5" class="py-12 px-6 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span>No users registered in the system.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <div id="modalCreate" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-xl w-full max-w-md overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalCreateContentWrapper">
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-white">Tambah User</h3>
    ...
    @push('myscript')
        <script>
            ...
            function openModalCreate() {
                ...
                $('#loadCreateContent').load('/users/create', function() {
                    initFormValidation('#formCreateUser');
                });
            }

            function openModalEdit(id) {
                openModal('modalEdit');
                $('#loadEditContent').load('/users/' + id + '/edit', function() {
                    initFormValidation('#formEditUser');
                });
            }

            // Real-time validation for User forms
            function initFormValidation(formSelector) {
                ...
                function validateField(field) {
                    ...
                    } else if (input.attr('type') === 'password' && input.attr('required') && value.length < 8) {
                        isValid = false;
            ...
            $(document).on('click', '.btn-delete', function(e) {
                ...
                Swal.fire({
                    title: 'Hapus User?',
                    text: "Apakah Anda yakin ingin menghapus user '" + name + "' secara permanen?",
                    icon: 'warning',
