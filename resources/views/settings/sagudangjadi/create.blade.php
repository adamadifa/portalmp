<x-app-layout>
    <x-slot name="header">
        Buat Saldo Awal Gudang Jadi
    </x-slot>

    <!-- Header Actions -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Buat Saldo Awal Gudang Jadi</h2>
            <p class="text-sm text-gray-500 mt-1">Isi periode bulan dan tahun lalu klik "Get Saldo" untuk mengisi saldo awal produk.</p>
        </div>
        <div>
            <a href="{{ route('sagudangjadi.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition shadow-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="lg:col-span-2">
            <form action="{{ route('sagudangjadi.store') }}" method="POST" id="formCreatesaldoawal" class="space-y-6">
                @csrf
                
                <!-- Period selection (Standard style matching filter form) -->
                <div class="mb-4">
                    <div class="flex flex-col sm:flex-row gap-2 w-full items-center">
                        ...
                        <div class="w-full sm:w-auto">
                            <button type="button" id="getsaldo" class="w-full sm:w-auto inline-flex items-center justify-center px-5 py-3 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition shadow-sm whitespace-nowrap h-[40px]">
                ...
                <!-- Detail list card (Styled like /barang) -->
        ...
        <div class="space-y-6">
            <div class="bg-gradient-to-br from-[#002e65] to-[#294C9A] p-6 rounded-2xl text-white shadow-md">
                ...
                <p class="text-xs text-blue-100 leading-relaxed mb-4">
                    Ikuti instruksi berikut untuk mendaftarkan saldo awal persediaan barang jadi:
                </p>
                <div class="space-y-3">
    ...
    <script>
        $(function() {
            function loaddetailsaldo() {
                ...
                if (bulan == "") {
                    Swal.fire({
                        ...
                        customClass: {
                            confirmButton: 'inline-flex items-center justify-center px-4 py-2 text-sm font-semibold text-white bg-[#294C9A] rounded-xl hover:bg-[#1E3A70]'
                        },
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-sm text-gray-400">
                                <div class="flex justify-center items-center gap-2">
                                    <div class="animate-spin rounded-full h-5 w-5 border-b-2 border-[#294C9A]"></div>
                    ...
                    $.ajax({
                        ...
                        success: function(respond) {
                            if (respond === '1') {
                                ...
                                $("#loaddetailsaldo").html(`
                                    <tr>
                                        <td colspan="3" class="px-6 py-8 text-center text-sm text-red-500 font-semibold">
                                            Gagal: Saldo Bulan Sebelumnya Belum Diinput!
                                        </td>
                                    </tr>
                                `);
                            } else {
            ...
            $("#formCreatesaldoawal").submit(function(e) {
                ...
                if (form.find('#loaddetailsaldo tr').length == 0 || form.find('#loaddetailsaldo input[name="kode_produk[]"]').length == 0) {
                    Swal.fire({
                        title: "Oops!",
