@foreach ($barang as $d)
    ...
    <tr class="odd:bg-white even:bg-gray-50/50 hover:bg-gray-100/50 transition-colors">
        ...
        <td class="py-2 px-4 text-gray-500 font-medium">{{ $d->satuan }}</td>
        <td class="py-2 px-4 text-right">
            @if ($readonly)
                <input type="hidden" name="harga[]" value="{{ formatAngkaDesimal($jumlah_saldo_akhir) }}">
                <span class="font-bold text-gray-900">{{ formatAngkaDesimal($jumlah_saldo_akhir) }}</span>
            @else
                <input type="text" name="harga[]" value="{{ formatAngkaDesimal($jumlah_saldo_akhir) }}"
                    class="w-28 text-right text-xs font-bold text-gray-800 border border-gray-200 focus:ring-1 focus:ring-[#294C9A] focus:border-[#294C9A] rounded-lg px-2 py-1 focus:outline-none money">
            @endif
        </td>
    </tr>
@endforeach
