<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
        <div>
            <span class="block text-[10px] uppercase font-bold text-gray-400">Kode Saldo Awal</span>
            <span class="text-sm font-bold text-gray-800">{{ $saldo_awal->kode_saldo_awal }}</span>
        </div>
        <div>
            <span class="block text-[10px] uppercase font-bold text-gray-400">Bulan</span>
            <span class="text-sm font-bold text-gray-800">{{ $nama_bulan[$saldo_awal->bulan] }}</span>
        </div>
        <div>
            <span class="block text-[10px] uppercase font-bold text-gray-400">Tahun</span>
            <span class="text-sm font-bold text-[#294C9A]">{{ $saldo_awal->tahun }}</span>
        </div>
        <div>
            <span class="block text-[10px] uppercase font-bold text-gray-400">Tanggal</span>
            <span class="text-sm font-bold text-gray-800">{{ date('d-m-Y', strtotime($saldo_awal->tanggal)) }}</span>
        </div>
    </div>
                ...
                <tbody class="text-sm divide-y divide-gray-100">
                    @foreach ($detail as $d)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="py-2 px-4 text-right text-gray-900 font-bold">{{ formatAngkaDesimal($d->harga) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
