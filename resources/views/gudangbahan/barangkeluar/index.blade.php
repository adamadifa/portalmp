<x-app-layout>
    ...
    <!-- Header & Subtitle -->
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Barang Keluar Gudang Bahan</h2>
        <p class="text-sm text-gray-500 mt-1">Mengelola transaksi pencatatan barang keluar dari gudang bahan.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div class="lg:col-span-2 space-y-6">
            <!-- Filter -->
            <div class="mb-4">
                <form action="{{ route('barangkeluargudangbahan.index') }}" method="GET" class="flex flex-col sm:flex-row flex-wrap gap-2 w-full items-center">
                    ...
                    <div class="w-full sm:w-auto flex-1 min-w-[160px]">
                        <select name="kode_jenis_pengeluaran_search" id="kode_jenis_pengeluaran_search" class="block w-full py-2.5 px-4 text-xs text-gray-900 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#294C9A] focus:border-[#294C9A] focus:outline-none transition shadow-sm">
                            <option value="">Jenis Pengeluaran</option>
                            @foreach ($list_jenis_pengeluaran as $d)
                                <option value="{{ $d['kode_jenis_pengeluaran'] }}" {{ Request('kode_jenis_pengeluaran_search') == $d['kode_jenis_pengeluaran'] ? 'selected' : '' }}>
                                    {{ $d['jenis_pengeluaran'] }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10">
                    <div class="flex items-center gap-2 text-white">
                        ...
                        <h3 class="text-sm font-semibold">Data Barang Keluar</h3>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm divide-y divide-gray-100">
                            ...
                            <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100/50 transition-colors">
                                ...
                                <td class="py-2.5 px-4 text-gray-700 font-medium">{{ $jenis_pengeluaran[$d->kode_jenis_pengeluaran] ?? $d->kode_jenis_pengeluaran }}</td>
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="py-10 px-4 text-center text-sm text-gray-400">Belum ada data barang keluar.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div>
            <div class="bg-gradient-to-br from-[#002e65] to-[#294C9A] p-6 rounded-2xl text-white shadow-md">
                <h4 class="font-bold text-lg mb-3">Barang Keluar</h4>
                <p class="text-xs text-blue-100 leading-relaxed mb-4">
                    Pencatatan mutasi barang keluar digunakan untuk mendokumentasikan pengeluaran bahan baku/penolong untuk proses produksi, pengiriman ke cabang, maupun pemakaian lainnya.
                </p>
                <div class="space-y-3">
                    <div class="flex items-start gap-3">
                        <div class="w-5 h-5 rounded-full bg-white/20 flex items-center justify-center font-bold text-xs shrink-0">1</div>
                        <span class="text-xs text-blue-100">Gunakan filter pencarian di samping untuk menyaring data berdasarkan periode tanggal atau jenis pengeluaran.</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="modalCreate" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-5xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalCreateContentWrapper">
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10 text-white">
                <h3 class="text-sm font-bold">Tambah Barang Keluar</h3>
    ...
    <div id="modalEdit" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-5xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalEditContentWrapper">
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10 text-white">
                <h3 class="text-sm font-bold">Edit Barang Keluar</h3>
    ...
    <div id="modalShow" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-4xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalShowContentWrapper">
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10 text-white">
                <h3 class="text-sm font-bold">Detail Barang Keluar</h3>
                <button onclick="closeModal('modalShow')" class="text-white/80 hover:text-white" type="button">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>
            <div class="p-6 max-h-[85vh] overflow-y-auto" id="modalShowContent">
                <div class="flex justify-center items-center py-12">
                    <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-[#294C9A]"></div>
                </div>
            </div>
    ...
    <script>
        ...
        function openModal(modalId) {
            ...
            const wrapper = document.getElementById(modalId + 'ContentWrapper');
        ...
        function openModalCreate() {
            ...
            $.get("{{ route('barangkeluargudangbahan.create') }}", function(response) {
                $('#modalCreateContent').html(response);
            }).fail(function() {
                $('#modalCreateContent').html('<div class="text-center py-8 text-red-500 font-semibold">Gagal memuat form tambah data.</div>');
            });
        }

        function openModalEdit(id) {
            openModal('modalEdit');
            $('#modalEditContent').html('<div class="flex justify-center items-center py-12"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-[#294C9A]"></div></div>');
            let url = "{{ route('barangkeluargudangbahan.edit', ':id') }}".replace(':id', id);
            $.get(url, function(response) {
                $('#modalEditContent').html(response);
            }).fail(function() {
                $('#modalEditContent').html('<div class="text-center py-8 text-red-500 font-semibold">Gagal memuat form edit data.</div>');
            });
        }
        ...
        function openModalShow(id) {
            ...
            let url = "{{ route('barangkeluargudangbahan.show', ':id') }}".replace(':id', id);
            $.get(url, function(response) {
                $('#modalShowContent').html(response);
            }).fail(function() {
                $('#modalShowContent').html('<div class="text-center py-8 text-red-500 font-semibold">Gagal memuat detail data.</div>');
            });
        }
        ...
        $(document).on('click', '.btn-delete', function(e) {
            ...
            let deleteUrl = "{{ route('barangkeluargudangbahan.delete', ':id') }}".replace(':id', code);

            Swal.fire({
                title: 'Apakah Anda Yakin?',
                text: "Data barang keluar " + name + " akan dihapus permanen!",
                icon: 'warning',
                ...
                customClass: {
                    confirmButton: 'inline-flex items-center px-4 py-2 text-sm font-semibold text-white bg-red-600 rounded-md mr-2',
                    cancelButton: 'inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 bg-gray-100 rounded-md'
                },
