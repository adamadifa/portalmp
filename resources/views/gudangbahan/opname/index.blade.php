<x-app-layout>
    ...
    <!-- Header & Subtitle -->
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Opname Gudang Bahan</h2>
        <p class="text-sm text-gray-500 mt-1">Mengelola data stok opname barang di gudang bahan.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div class="lg:col-span-2 space-y-6">
            <!-- Filter -->
            <div class="mb-4">
                <form action="{{ route('opgudangbahan.index') }}" method="GET" class="flex flex-col sm:flex-row gap-2 w-full">
                    <div class="flex-1">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10">
                    <div class="flex items-center gap-2 text-white">
                        ...
                        <h3 class="text-sm font-semibold">Data Opname</h3>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="text-xs font-semibold uppercase tracking-wider bg-[#294C9A] text-white">
                                <th class="py-3 px-4">Kode Opname</th>
                        ...
                        <tbody class="text-sm divide-y divide-gray-100">
                            ...
                            <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100/50 transition-colors">
                                <td class="py-2.5 px-4 font-bold text-[#294C9A]">{{ $d->kode_opname }}</td>
                                ...
                                <td class="py-2.5 px-4 font-medium text-center">
                                    <div class="flex items-center justify-center gap-3">
                                        ...
                                        <button type="button" onclick="openModalShow('{{ Crypt::encrypt($d->kode_opname) }}')" class="p-1.5 text-cyan-600 hover:bg-cyan-50 rounded-lg transition" title="Detail">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                        </button>
                                        @endcan

                                        @can('opgudangbahan.edit')
                                        <button type="button" onclick="openModalEdit('{{ Crypt::encrypt($d->kode_opname) }}')" class="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg transition" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </button>
                                        @endcan

                                        @can('opgudangbahan.delete')
                                        <button type="button" class="btn-delete p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition" data-code="{{ Crypt::encrypt($d->kode_opname) }}" data-name="{{ $d->kode_opname }}" title="Hapus">
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="py-10 px-4 text-center text-sm text-gray-400">Belum ada data opname.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div>
            <div class="bg-gradient-to-br from-[#002e65] to-[#294C9A] p-6 rounded-2xl text-white shadow-md">
                <h4 class="font-bold text-lg mb-3">Opname Gudang Bahan</h4>
                <p class="text-xs text-blue-100 leading-relaxed mb-4">
                    Stok opname digunakan untuk melakukan audit/penyesuaian jumlah fisik bahan baku riil di gudang bahan pada akhir periode laporan tertentu.
                </p>
                <div class="space-y-3">
                    <div class="flex items-start gap-3">
                        <div class="w-5 h-5 rounded-full bg-white/20 flex items-center justify-center font-bold text-xs shrink-0">1</div>
                        <span class="text-xs text-blue-100">Pilih Bulan & Tahun lalu tekan Cari untuk menampilkan riwayat opname.</span>
                    </div>
                    <div class="flex items-start gap-3">
                        <div class="w-5 h-5 rounded-full bg-white/20 flex items-center justify-center font-bold text-xs shrink-0">2</div>
                        <span class="text-xs text-blue-100">Tekan "Tambah Data" untuk menginput hasil opname fisik baru.</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="modalCreate" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-4xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalCreateContentWrapper">
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10 text-white">
                <h3 class="text-sm font-bold">Tambah Data Opname</h3>
    ...
    <div id="modalEdit" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-4xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalEditContentWrapper">
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10 text-white">
                <h3 class="text-sm font-bold">Edit Data Opname</h3>
    ...
    <div id="modalShow" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-4xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalShowContentWrapper">
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-sm font-bold text-gray-900">Rincian Opname</h3>
    ...
    <script>
        $(function() {
            ...
            window.openModalCreate = function() {
                ...
                $("#modalCreateContent").load(`/opgudangbahan/create`);
            }

            window.openModalEdit = function(code) {
                openModal('modalEdit');
                $("#modalEditContent").load(`/opgudangbahan/${code}/edit`);
            }

            window.openModalShow = function(code) {
                openModal('modalShow');
                $("#modalShowContent").load(`/opgudangbahan/${code}/show`);
            }
            ...
            $(document).on('click', '.btn-delete', function(e) {
                ...
                Swal.fire({
                    ...
                    text: `Data opname "${name}" akan dihapus secara permanen!`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        form.method = 'POST';
                        form.action = `/opgudangbahan/${code}`;
                        form.innerHTML = `
                            @csrf
                            @method('DELETE')
                        `;
