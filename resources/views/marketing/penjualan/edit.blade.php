<x-app-layout>
    <x-slot name="header">
        Edit Penjualan Marketing
    </x-slot>

    <!-- Header Actions -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Penjualan Marketing</h2>
            <p class="text-xs text-gray-500 mt-1">Formulir penyesuaian transaksi penjualan produk ke Portax.</p>
        </div>
        <!-- Breadcrumbs -->
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 text-xs font-semibold text-gray-500">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="text-[#294C9A] font-bold">Edit</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <form action="{{ route('penjualanmarketing.update', Crypt::encrypt($penjualan->no_bukti)) }}" method="POST" id="formPenjualan" class="w-full" novalidate>
        @csrf
        @method('PUT')
                ...
                <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden min-h-[350px] flex flex-col justify-between">
                    <div>
                        <div class="px-5 py-4 flex items-center justify-between bg-[#294C9A] text-white shrink-0">
                            ...
                            <span class="px-3 py-1 bg-white/10 text-white border border-white/20 rounded-full text-xs font-bold font-mono" id="item_count_badge">{{ count($detail) }} Item</span>
                        </div>
                            ...
                            <table class="w-full text-xs text-left text-gray-600 border-collapse" id="tabelproduk">
                                ...
                                <tbody id="loadproduk" class="divide-y divide-gray-50 text-gray-700 bg-white">
                                    ...
                                    @forelse ($detail as $d)
                                        ...
                                        <tr id="row_{{ $d->kode_produk }}" class="hover:bg-gray-55 transition">
                                            ...
                                            <td class="py-3 px-6 text-right font-medium">
                                                Rp {{ number_format($d->harga_dus, 0, ',', '.') }}
                                            ...
                                            <td class="py-3 px-6 text-right font-bold text-gray-900">
                                                Rp <span class="row_subtotal_text">{{ number_format($d->subtotal, 0, ',', '.') }}</span>
                                                <input type="hidden" name="subtotal[]" class="row_subtotal" value="{{ $d->subtotal }}">
                                            </td>
                                            <td class="py-3 px-6 text-center">
                                                <button type="button" onclick="hapusRow('{{ $d->kode_produk }}')" class="text-red-500 hover:text-red-700 p-1.5 hover:bg-red-50 rounded-xl transition">
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-[#294C9A] text-white rounded-3xl p-6 shadow-md relative overflow-hidden">
                    ...
                    <div class="relative z-10 space-y-2">
                        ...
                        <div class="text-4xl font-extrabold tracking-tight font-mono break-all" id="total_pembelian_text">Rp {{ number_format($total, 0, ',', '.') }}</div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm space-y-6">
                    ...
                    <div>
                        <div class="c-fl-group">
                            ...
                            <input type="text" name="no_bukti" id="no_bukti" value="{{ $penjualan->no_bukti }}" readonly class="fi cursor-not-allowed text-gray-400 bg-gray-100/50" autocomplete="off" />
                    ...
                    <div>
                        <div class="c-fl-group">
                            ...
                            <input type="text" name="tanggal" id="tanggal" value="{{ $penjualan->tanggal }}" required class="fi flatpickr-date font-bold text-gray-800" placeholder="Tanggal" autocomplete="off" />
                    ...
                    <div>
                        <div class="c-fl-group">
                            ...
                            <select name="jenis_transaksi" id="jenis_transaksi" required class="fi font-bold text-gray-800">
                                ...
                                <option value="T" {{ $penjualan->jenis_transaksi == 'T' ? 'selected' : '' }}>TUNAI</option>
                                <option value="K" {{ $penjualan->jenis_transaksi == 'K' ? 'selected' : '' }}>KREDIT</option>
                            </select>
                    ...
                    <!-- Jenis Bayar -->
                    <div id="wrapper_jenis_bayar" class="c-fl-group {{ $penjualan->jenis_transaksi == 'T' ? '' : 'hidden' }}">
                        <div class="c-fl-group !mt-0">
                            <span class="c-fl-icon">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
                            </span>
                            <select name="jenis_bayar" id="jenis_bayar" class="fi font-bold text-gray-800" {{ $penjualan->jenis_transaksi == 'T' ? 'required' : '' }}>
                                <option value="">Jenis Bayar</option>
                                <option value="TN" {{ $penjualan->jenis_bayar == 'TN' ? 'selected' : '' }}>CASH</option>
                                <option value="TR" {{ $penjualan->jenis_bayar == 'TR' ? 'selected' : '' }}>TRANSFER</option>
                            </select>
