<x-app-layout>
    <x-slot name="header">
        Bukti Penyerahan Barang Jadi (BPBJ)
    </x-slot>
    ...
    <!-- Header & Subtitle -->
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-900 tracking-tight">Bukti Penyerahan Barang Jadi (BPBJ)</h2>
        <p class="text-sm text-gray-500 mt-1">Mengelola data bukti penyerahan barang jadi.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div class="lg:col-span-2 space-y-6">
            <!-- Filter -->
            <div class="mb-4">
                <form action="{{ route('bpbj.index') }}" method="GET" class="flex flex-col sm:flex-row gap-2 w-full">
                    <div class="flex-1">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10">
                    <div class="flex items-center gap-2 text-white">
                        ...
                        <h3 class="text-sm font-semibold">Data Bukti Penyerahan Barang Jadi (BPBJ)</h3>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="text-xs font-semibold uppercase tracking-wider bg-[#294C9A] text-white">
                                <th class="py-3 px-4" style="width: 40%;">No. BPBJ</th>
                                <th class="py-3 px-4" style="width: 40%;">Tanggal</th>
                        ...
                        <tbody class="text-sm divide-y divide-gray-100">
                            ...
                            @empty
                            <tr>
                                <td colspan="3" class="py-10 px-4 text-center text-sm text-gray-400">Belum ada data BPBJ.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div>
            <div class="bg-gradient-to-br from-[#002e65] to-[#294C9A] p-6 rounded-2xl text-white shadow-md">
                <h4 class="font-bold text-lg mb-3">Informasi BPBJ</h4>
                <p class="text-xs text-blue-100 leading-relaxed mb-4">
                    Mengelola Bukti Penyerahan Barang Jadi dari bagian produksi ke bagian gudang.
                </p>
                <div class="space-y-3">
                    <div class="flex items-start gap-3">
                        <div class="w-5 h-5 rounded-full bg-white/20 flex items-center justify-center font-bold text-xs shrink-0">1</div>
                        <span class="text-xs text-blue-100">Klik "Tambah BPBJ" untuk membuka form pembuatan data.</span>
                    </div>
                    <div class="flex items-start gap-3">
                        <div class="w-5 h-5 rounded-full bg-white/20 flex items-center justify-center font-bold text-xs shrink-0">2</div>
                        <span class="text-xs text-blue-100">Gunakan icon mata untuk melihat rincian produk yang diserahkan.</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="modalCreate" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-4xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalCreateContentWrapper">
            <div class="bg-[#294C9A] px-6 py-4 flex items-center justify-between border-b border-white/10 text-white">
                <h3 class="text-sm font-bold">Tambah Bukti Penyerahan Barang Jadi (BPBJ)</h3>
    ...
    <div id="modalShow" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-2xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300" id="modalShowContentWrapper">
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-sm font-bold text-gray-900">Rincian BPBJ</h3>
    ...
    <script>
        $(function() {
            ...
            $(document).on('click', '.btn-delete', function(e) {
                ...
                Swal.fire({
                    title: 'Hapus BPBJ?',
                    text: "Apakah Anda yakin ingin menghapus data BPBJ '" + name + "'?",
                    icon: 'warning',
