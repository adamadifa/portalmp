@foreach ($produk as $d)
    <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100/50 transition-colors">
        ...
        <td class="px-4 py-1.5 text-right">
            @if ($readonly)
                <input type="hidden" name="jumlah[]" value="{{ empty($d->saldo_akhir) ? 0 : formatAngka($d->saldo_akhir) }}">
            ...
            @else
                <input type="text" name="jumlah[]" value="{{ isset($d->saldo_akhir) ? formatAngka($d->saldo_akhir) : '' }}"
