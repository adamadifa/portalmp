...
class SamutasiproduksiController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('produksi.samutasiproduksi.index', compact('list_bulan', 'start_year', 'saldo_awal', 'nama_bulan'));
    }
    ...
    public function create()
    {
        $list_bulan = config('global.list_bulan');
        $start_year = config('global.start_year');
        return view('produksi.samutasiproduksi.create', compact('list_bulan', 'start_year'));
    }
    ...
    public function show($kode_saldo_awal)
    {
        ...
        $detail = Detailsaldoawalmutasiproduksi::where('kode_saldo_awal', $kode_saldo_awal)
            ...
            ->get();
        $nama_bulan = config('global.nama_bulan');
        return view('produksi.samutasiproduksi.show', compact('saldo_awal', 'nama_bulan', 'detail'));
    }
    ...
    public function getdetailsaldo(Request $request)
    {
        ...
        if (empty($ceksaldobulanlalu) && !empty($ceksaldobulanini)) {
            ...
            // Jika Saldo Bulan Lalu Ada Maka Hitung Saldo Awal Bulan Lalu + Sisa Mutasi Bulan Lalu
            $produk = Produk::selectRaw(
                'produk.kode_produk,
                nama_produk,
        ...
        if (empty($ceksaldo)) {
            $readonly = false;
            return view('produksi.samutasiproduksi.getdetailsaldo', compact($data));
        } else {
            if (empty($ceksaldobulanlalu) && empty($ceksaldobulanini)) {
                return 1;
            } else {
