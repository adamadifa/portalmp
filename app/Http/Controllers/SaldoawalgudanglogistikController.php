...
class SaldoawalgudanglogistikController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!is_array($kode_barang) || !is_array($jumlah) || !is_array($harga) ||
            ...
            return Redirect::back()->with(messageError('Gagal menyimpan. Jumlah input barang, jumlah, dan harga tidak sama.'));
        }
    ...
    public function getdetailsaldo(Request $request)
    {
        ...
        $blnlalu = $bulanlalu < 10 ? "0" . $bulanlalu : $bulanlalu;

        $tgl_dari_bulanlalu = $tahunlalu . "-" . $blnlalu . "-01";
        ...
        if (empty($ceksaldo) && empty($ceksaldobulanlalu)) {
            $readonly = false;
            return view('gudanglogistik.saldoawal.getdetailsaldo', compact('barang', 'readonly'));
        } else {
            if (empty($ceksaldobulanlalu) && empty($ceksaldobulanini)) {
                return 1;
            } else {
